...
import type { CheckoutBody, MongoProduct, OrderItem } from "@/types/store";
...
export async function POST(req: NextRequest, context: CheckoutContext) {
  try {
    await connectDB();
    ...
    if (!body.buyer?.name || !body.buyer?.email || !body.buyer?.phone) {
      return NextResponse.json(
    ...
    const safeItems: OrderItem[] = cartItems.map((cartItem) => {
      const product = dbProducts.find(
        (dbProduct) => String(dbProduct._id) === String(cartItem.productId)
      );
      ...
      const variant = variantSku
        ? product.variants?.find((item) => item.sku === variantSku)
        : undefined;
      ...
      if (!variant && Number(product.stock || 0) < quantity) {
        throw new Error("Stock insuficiente");
      }
    ...
    const shippingTotal = Number(body.shipping?.quote?.price || 0);
    ...
    const order = await Order.create({
      ...
      shipping: {
        ...
        provider: body.shipping?.quote?.provider,
        service: body.shipping?.quote?.service,
        price: shippingTotal,
        eta: body.shipping?.quote?.eta,
        postalCodeOrigin: body.shipping?.quote?.postalCodeOrigin,
        postalCodeDestination: body.shipping?.quote?.postalCodeDestination,
      },
    });
    ...
    const mpPreference = await preference.create({
      body: {
        items: safeItems.map((item) => ({
          id: item.variantSku || item.productId,
          title: item.title,
